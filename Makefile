...
# =====================================
IMAGE_NAME = curetrack-web
CONTAINER_NAME = curetrack-web
PORT = 8080
...
# Run with hot reload (mount local files)
run: ## Run container with hot reload (mounted volumes)
	...
	docker run -d -p $(PORT):80 --name $(CONTAINER_NAME) \
		-v "$(CURDIR)/index.html:/usr/share/nginx/html/index.html" \
		-v "$(CURDIR)/learn-more.html:/usr/share/nginx/html/learn-more.html" \
		-v "$(CURDIR)/assets:/usr/share/nginx/html/assets" \
		$(IMAGE_NAME)
